<?php
/*
  Plugin Name: Wpcontroller worker
  Plugin URI:
...
function addTrackingCode() {
    if (get_option('_piwik_domain_id')) {
        ...
        <script type="text/javascript">
            ...
            (function() {
                var u = (("https:" == document.location.protocol) ? "https" : "http") + "://my.uptimeweb.co.uk/analytics/";
                _paq.push(["setTrackerUrl", u + "piwik.php"]);
                _paq.push(["setSiteId", "<?php echo get_option('_piwik_domain_id') ?>"]);
        ...
        <noscript><p><img src="http://my.uptimeweb.co.uk/analytics/piwik.php?idsite=<?php echo get_option('_piwik_domain_id'); ?>" style="border:0;" alt="" /></p></noscript>
        <!-- End Piwik Code -->
        <?php
    }
}
...
if(!function_exists('mmb_get_site_info')) {
    function mmb_get_site_info($params) {
        $plugins = get_option('_site_transient_update_plugins');
        $data['plugins']['installed'] = count($plugins->checked);
        $data['plugins']['updates_available'] = count($plugins->response);

        $data['plugins']['active'] = count(get_option('active_plugins'));

        $themes = get_option('_site_transient_update_themes');

        $data['themes']['installed'] = count($themes->checked);
        $data['themes']['updates_available'] = count($themes->response);

        $core = get_option('_site_transient_update_core');
        $data['core']['installed_version'] = $core->version_checked;
        $data['core']['current_version'] = $core->updates[0]->current;
