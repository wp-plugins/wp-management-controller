...
define('MWP_BACKUP_DIR', WP_CONTENT_DIR . '/wpcontroller-worker/backups');
...
$zip_errors = array('No error', 'No error', 'Unexpected end of zip file', 'A generic error in the zipfile format was detected', 'zip was unable to allocate itself memory', 'A severe error in the zipfile format was detected', 'Entry too large to be split with zipsplit', 'Invalid comment format', 'zip -T failed or out of memory', 'The user aborted zip prematurely', 'zip encountered an error while using a temp file. Please check if there is enough disk space', 'Read or seek error', 'zip has nothing to do', 'Missing or empty zip file', 'Error writing to a file. Please check if there is enough disk space', 'zip was unable to create a file to write to', 'bad command line parameters', 'no error', 'zip could not open a specified file to read');
...
/**
 * The main class for processing database and full backups on WpController worker.
 *
 * @copyright   2011-2012 Prelovac Media
 * @version     3.9.24
 * @package     WpController
 * @subpackage  backup
 *
 */
class MMB_Backup extends MMB_Core
{
    ...
    function __construct() {
        ...
        $this->statuses = array('db_dump' => 1, 'db_zip' => 2, 'files_zip' => 3, 's3' => 4, 'dropbox' => 5, 'ftp' => 6, 'email' => 7, 'google_drive' => 8, 'finished' => 100);
    ...
    function set_memory() {
        ...
        if ($last == 'g') $memory_limit = ((int)$memory_limit) * 1024;
        else if ($last == 'm') $memory_limit = (int)$memory_limit;
        if ($last == 'k') $memory_limit = ((int)$memory_limit) / 1024;
    ...
    function get_backup_settings() {
        ...
        if (!empty($backup_settings)) return $backup_settings;
        else return false;
    }
    ...
    function set_backup_task($params) {
        ...
        if (!empty($params)) {
            ...
            $before = $this->tasks;
            if (!$before || empty($before)) $before = array();
            ...
            if (isset($args['remove'])) {
                ...
                if (isset($params['account_info']) && is_array($params['account_info'])) {
                    ...
                    //only if sends from master first time(secure data)
                    $args['account_info'] = $account_info;
                }
                
                $before[$task_name]['task_args'] = $args;
                if (strlen($args['schedule'])) $before[$task_name]['task_args']['next'] = $this->schedule_next($args['type'], $args['schedule']);
            ...
            if (isset($time) && $time) {
                ...
                //set next result time before backup
    ...
    /**
     * Runs backup task invoked from WpController master.
     *
    ...
    function task_now($task_name, $google_drive_token = false) {
        ...
        $this->set_backup_task(array('task_name' => $task_name, 'args' => $settings[$task_name]['task_args'], 'time' => time()));
        ...
        //Check for error
        if (is_array($result) && array_key_exists('error', $result)) {
            $this->set_backup_task(array('task_name' => $task_name, 'args' => $settings[$task_name]['task_args'], 'error' => $result['error']));
            return $result;
        } else {
     ...
     * All backups are compressed by zip and placed in wp-content/wpcontroller-worker/backups folder.
     *
    ...
    function backup($args, $task_name = false) {
        if (!$args || empty($args)) return false;
        ...
        if (!empty($account_info)) {
            $found = false;
            $destinations = array('mwp_ftp', 'mwp_sftp', 'mwp_amazon_s3', 'mwp_dropbox', 'mwp_google_drive', 'mwp_email', 'mwp_copy');
        ...
        if (!file_exists($new_file_path)) {
            if (!mkdir($new_file_path, 0755, true)) return array('error' => 'Permission denied, make sure you have write permissions to the wp-content folder.');
        }
        
        @file_put_contents($new_file_path . '/index.php', '');
        ...
        $backup_url = WP_CONTENT_URL . '/wpcontroller-worker/backups/' . $this->site_name . '_' . $label . '_' . $what . '_' . date('Y-m-d') . '_' . $hash . '.zip';
        ...
        if ($task_name) {
            ...
            if (isset($backup_settings[$task_name]['task_args']['account_info']['mwp_copy'])) {
                $paths['copy'] = basename($backup_url);
            }
    ...
    function backup_full($task_name, $backup_file, $exclude = array(), $include = array()) {
        ...
        if (!$zip_db_result) {
            ...
            if (!$zip_archive_db_result) {
                ...
                if (!$pclzip_db_result) {
                    ...
                    return array('error' => 'Failed to zip database. ' . $archive->error_code . $archive->error_string);
                }
            }
        }
        ...
        $remove = array(trim(basename(WP_CONTENT_DIR)) . "/wpcontroller-worker/backups", trim(basename(WP_CONTENT_DIR)) . "/" . md5('mmb-worker') . "/mwp_backups", trim(basename(WP_CONTENT_DIR)) . "/cache", trim(basename(WP_CONTENT_DIR)) . "/w3tc",);
        ...
        if (!$zip_result) {
            ...
            if (!$zip_archive_result) {
                ...
                if (!$pclzip_result) {
                    ...
                    if (!$pclzip_result) {
                        @unlink($backup_file);
                        return array('error' => 'Failed to zip files. pclZip error (' . $archive->error_code . '): .' . $archive->error_string);
                    }
                }
            }
        }
    ...
    function zip_archive_backup_db($task_name, $db_result, $backup_file) {
        ...
        $zip = new ZipArchive();
        $result = $zip->open($backup_file, ZIPARCHIVE::OVERWRITE);
        
        // Tries to open $backup_file for acrhiving
        if ($result === true) {
            $result = $result && $zip->addFile(MWP_BACKUP_DIR . '/mwp_db/index.php', "mwp_db/index.php");
            
            // Tries to add mwp_db/index.php to $backup_file
            $result = $result && $zip->addFile($db_result, "mwp_db/" . basename($db_result));
            
            // Tries to add db dump form mwp_db dir to $backup_file
            $result = $result && $zip->close();
            
            // Tries to close $backup_file
        ...
        // true if $backup_file iz zipped successfully, false if error is occured in zip process
    ...
    function zip_backup($task_name, $backup_file, $exclude, $include) {
        ...
        if (!empty($exclude)) {
            foreach ($exclude as $data) {
                if (is_dir(ABSPATH . $data)) {
                    if ($sys == 'WIN') $exclude_data.= " $data/*.*";
                    else $exclude_data.= " '$data/*'";
                } else {
                    if ($sys == 'WIN') {
        ...
        $include_data = ". -i";
        foreach ($add as $data) {
            if ($sys == 'WIN') $include_data.= " $data/*.*";
            else $include_data.= " '$data/*'";
        }
        ...
        if (!$result_f || $result_f == 18) {
            ...
            // disregard permissions error, file can't be accessed
            ...
            if ($result_d && $result_d != 18) {
                @unlink($backup_file);
                if ($result_d > 0 && $result_d < 18) return array('error' => 'Failed to archive files (' . $zip_errors[$result_d] . ') .');
                else {
    ...
    function zip_archive_backup($task_name, $backup_file, $exclude, $include, $overwrite = false) {
        ...
        if ($result === true) {
            foreach ($filelist as $file) {
                $result = $result && $zip->addFile($file, sprintf("%s", str_replace(ABSPATH, '', $file)));
                
                // Tries to add a new file to $backup_file
    ...
    function pclzip_backup($task_name, $backup_file, $exclude, $include) {
        ...
        if (!empty($include)) {
            foreach ($include as $data) {
                if ($data && file_exists(ABSPATH . $data)) $include_data[] = ABSPATH . $data . '/';
            }
        }
        ...
        if (!empty($exclude)) {
            foreach ($exclude as $data) {
                if (file_exists(ABSPATH . $data)) {
                    if (is_dir(ABSPATH . $data)) $exclude_data[] = $data . '/';
                    else $exclude_data[] = $data;
                }
            }
        }
        ...
        if (!$zip_db_result) {
            ...
            if (!$zip_archive_db_result) {
                ...
                if (!$pclzip_db_result) {
                    ...
                    return array('error' => 'Failed to zip database. pclZip error (' . $archive->error_code . '): .' . $archive->error_string);
                }
            }
        }
    ...
    function backup_db() {
        ...
        if (!file_exists($db_folder)) {
            if (!mkdir($db_folder, 0755, true)) return array('error' => 'Error creating database backup folder (' . $db_folder . '). Make sure you have corrrect write permissions.');
        }
        
        $file = $db_folder . DB_NAME . '.sql';
        $result = $this->backup_db_dump($file);
        
        // try mysqldump always then fallback to php dump
        return $result;
    }
    ...
    function backup_db_php($file) {
        ...
        foreach ($tables as $table) {
            ...
            if ($count > 100) $count = ceil($count / 100);
        ...
        if (filesize($file) == 0 || !is_file($file)) {
            @unlink($file);
            return array('error' => 'Database backup failed. Try to enable MySQL dump on your server.');
        }
    ...
    function restore($args) {
        ...
        if ($backup_url) {
            ...
            if (isset($args[$task_name]['task_args']['backup_from_server'])) {
                $backup_file = $task['task_results'][0]['server']['file_path'];
                $unlink_file = false;
                
                //Don't delete file if stored on server
                
                
            } elseif (isset($args['task_args']['backup_from_ftp'])) {
                $ftp_file = $args['task_args']['ftp'];
                $args = $args['task_args']['account_info']['mwp_ftp'];
            ...
            } elseif (isset($args['task_args']['backup_from_sftp'])) {
                $ftp_file = $args['task_args']['sftp'];
                $args = $args['task_args']['account_info']['mwp_sftp'];
                $args['backup_file'] = $ftp_file;
                $backup_file = $this->get_sftp_backup($args);
                
                if ($backup_file == false) {
                    return array('error' => 'Failed to download file from SFTP.');
                }
            } elseif (isset($args['task_args']['backup_from_amazons3'])) {
                $amazons3_file = $args['task_args']['amazons3'];
                $args = $args['task_args']['account_info']['mwp_amazon_s3'];
            ...
            } elseif (isset($args['task_args']['backup_from_dropbox'])) {
                $dropbox_file = $args['task_args']['dropbox'];
                $args = $args['task_args']['account_info']['mwp_dropbox'];
            ...
            } elseif (isset($args['task_args']['backup_from_google_drive'])) {
                $google_drive_file = $args['task_args']['google_drive'];
                $args = $args['task_args']['account_info']['mwp_google_drive'];
                ...
                if (is_array($backup_file) && isset($backup_file['error'])) {
                    return array('error' => 'Failed to download file from Google Drive, reason: ' . $backup_file['error']);
                } elseif ($backup_file == false) {
                    return array('error' => 'Failed to download file from Google Drive.');
                }
            } elseif (isset($args['task_args']['backup_from_copy'])) {
                $copy_file = $args['task_args']['backup_from_copy'];
                $args = $args['task_args']['account_info']['mwp_copy'];
                $args['backup_file'] = $copy_file;
                $backup_file = $this->get_copy_backup($args);
                
                if (is_array($backup_file) && isset($backup_file['error'])) {
                    return array('error' => 'Failed to download file from Copy.com');
                } elseif ($backup_file == false) {
                    return array('error' => 'Failed to download file from Copy.com.');
                }
            }
            $what = $args['task_args']['what'];
        }
        ...
        if ($backup_file && file_exists($backup_file)) {
            if ($overwrite) {
                ...
                if (!copy(ABSPATH . 'wp-config.php', ABSPATH . 'mwp-temp-wp-config.php')) {
                    @unlink($backup_file);
                    return array('error' => 'Error creating wp-config file.
    ...
    function optimize_tables($args) {
        ...
        global $wpdb;
        $wpdb->query("Delete from wp_posts where post_status='draft'");
        ...
        //extra operations
        if ($db_optimize_options['revisions'] == 'true') {
        ...
        // auto drafts
        if ($db_optimize_options['drafts'] == 'true') {
        ...
        // spam comments
        if ($db_optimize_options['spams'] == 'true') {
     ...
     function schedule_remote_backup($args) {
        global $wpdb;
        wp_clear_scheduled_hook('mwp_schedule_remote_backup');
        if (!wp_next_scheduled('mwp_schedule_remote_backup')) {
            wp_schedule_event(time(), 'tenminutes', 'mwp_schedule_remote_backup');
            update_option('mwp_schedule_remote_backup', $args);
        }
    }


    function check_scheduled_remote_backup(){
        $args = get_option('mwp_schedule_remote_backup');
        $params = $args;
        global $wpdb;
        extract($args);
        //previous schedule
        ...
        if (!$args['previous_schedule']) {
            $current_time = time();
            $diff =  $args['schedule_time'] - $args['scheduled_at_time'];
            $current_times_diff = $current_time - $args['scheduled_at_time'];
            if ($current_times_diff >= $diff) {
                $this->backup($params);
                $next_schedule = $current_time + $diff;
                $opt = $params;
                $opt['previous_schedule'] = $current_time;
                $opt['next_schedule'] = $next_schedule;
                $opt['diff'] = $diff;
                update_option('mwp_schedule_remote_backup', $opt);
            }
        }else{
            $current_time = time();
            if (($current_time + $args['diff']) >= $args['next_schedule']) {
                $this->backup($params);
                $next_schedule = $current_time + $args['diff'];
                $opt = $params;
                $opt['previous_schedule'] = $current_time;
                $opt['next_schedule'] = $next_schedule;
                $opt['diff'] = $args['diff'];
    ...
    function optimize_tables_scheduled($args) {
        global $wpdb;
        wp_clear_scheduled_hook('mwp_scheduled_optimize_tables');
        // if (!wp_next_scheduled('mwp_scheduled_optimize_tables')) {
            wp_schedule_event(time(), 'tenminutes', 'mwp_scheduled_optimize_tables');
            update_option('mwp_scheduled_optimize_tables', (array)$args);
        // }
    }

    function clean_optimize_schedule($args) {
        global $wpdb;
        wp_clear_scheduled_hook('mwp_scheduled_optimize_tables');
        delete_option('mwp_scheduled_optimize_tables');
    }
    
    function check_scheduled_optimize_tables() {
        global $wpdb;
        $args = get_option('mwp_scheduled_optimize_tables');
        $params = $args;
        extract($args);
        //previous schedule
        //next schedule time
        ...
        if (!$args['previous_schedule']) {
            ...
            $current_times_diff = $current_time - $args['scheduled_at_time'];
            if ($current_times_diff >= $diff) {
                $this->optimize_tables($params);
                $next_schedule = $current_time + $diff;
                $opt = $params;
                $opt['previous_schedule'] = $current_time;
                $opt['next_schedule'] = $next_schedule;
                $opt['diff'] = $diff;
                update_option('mwp_scheduled_optimize_tables', $opt);
            }
        }else{
            $current_time = time();
            if (($current_time + $args['diff']) >= $args['next_schedule']) {
    ...
    function check_mysql_paths() {
        ...
        if (substr(PHP_OS, 0, 3) == 'WIN') {
            ...
            if (empty($paths['mysql'])) $paths['mysql'] = 'mysql';
            
            // try anyway
            
            $paths['mysqldump'] = $this->mmb_exec('which mysqldump', true);
            if (empty($paths['mysqldump'])) $paths['mysqldump'] = 'mysqldump';
     ...
     */
    function check_sys() {
        if ($this->mmb_function_exists('exec')) return 'exec';
        
        if ($this->mmb_function_exists('system')) return 'system';
        
        if ($this->mmb_function_exists('passhtru')) return 'passthru';
    ...
    function email_backup($args) {
        $email = $args['email'];
        if (!is_email($email)) {
            return array('error' => 'Your email (' . $email . ') is not correct');
        }
        ...
        if (file_exists($backup_file) && $email) {
            ...
            $headers = 'From: WpController <no-reply@wpcontroller.co.uk>' . "\r\n";
            $subject = "WpController - " . $task_name . " - " . $this->site_name;
            ob_start();
        ...
        if (!$result) {
            return array('error' => 'Email not sent. Maybe your backup is too big for email or email server is not available on your website.');
        }
        return true;
    }
    ...
    function ftp_backup($args) {
        ...
        if ($ftp_ssl) {
            if (function_exists('ftp_ssl_connect')) {
                ...
                return array('error' => 'FTPS disabled: Please enable ftp_ssl_connect in PHP', 'partial' => 1);
            }
        } else {
            if (function_exists('ftp_connect')) {
                ...
                return array('error' => 'FTP disabled: Please enable ftp_connect in PHP', 'partial' => 1);
            }
        }
    ...
    function remove_ftp_backup($args) {
        ...
        if ($conn_id) {
            $login = @ftp_login($conn_id, $ftp_username, $ftp_password);
            if ($ftp_site_folder) $ftp_remote_folder.= '/' . $this->site_name;
    ...
    function remove_sftp_backup($args) {
        ...
        if (isset($backup_file) && isset($remote) && $backup_file !== "") $upload = $sftp->delete($remote . '/' . $backup_file);
        $sftp->disconnect();
    }
    ...
    function get_sftp_backup($args) {
        ...
        if ($ftp_site_folder) $remote.= '/' . $this->site_name;
     ...
     *
     * @param   array   $args   arguments passed to the function
     * [consumer_key] -> consumer key of WpController Dropbox application
     * [consumer_secret] -> consumer secret of WpController Dropbox application
     * [oauth_token] -> oauth token of user on WpController Dropbox application
     * [oauth_token_secret] -> oauth token secret of user on WpController Dropbox application
    ...
    function dropbox_backup($args) {
        ...
        if ($dropbox_site_folder == true) $dropbox_destination.= '/' . $this->site_name . '/' . basename($backup_file);
        else $dropbox_destination.= '/' . basename($backup_file);
    ...
    function remove_dropbox_backup($args) {
        ...
        if ($dropbox_site_folder == true) $dropbox_destination.= '/' . $this->site_name;
    ...
    function amazons3_backup($args) {
        if ($this->mmb_function_exists('curl_init')) {
            ...
            if ($as3_site_folder == true) $as3_directory.= '/' . $this->site_name;
    ...
    function google_drive_backup($args) {
        ...
        if (isset($files[0])) {
            $wpcontroller_folder = $files[0];
        }
        
        if (!isset($wpcontroller_folder)) {
            try {
                $_wpcontroller_folder = new Google_DriveFile();
                $_wpcontroller_folder->setTitle($google_drive_directory);
                $_wpcontroller_folder->setMimeType('application/vnd.google-apps.folder');
                
                if ($root_folder_id != null) {
                    $parent = new Google_ParentReference();
                    $parent->setId($root_folder_id);
                    $_wpcontroller_folder->setParents(array($parent));
                }
                
                $wpcontroller_folder = $gdrive_service->files->insert($_wpcontroller_folder, array());
            }
            catch(Exception $e) {
                return array('error' => $e->getMessage(),);
            }
        ...
        if ($google_drive_site_folder) {
            try {
                ...
                $wpcontroller_folder_id = $wpcontroller_folder->getId();
                $list_files = $gdrive_service->files->listFiles(array("q" => "title='$subfolder_title' and '$wpcontroller_folder_id' in parents and trashed = false"));
                $files = $list_files->getItems();
            }
            catch(Exception $e) {
                return array('error' => $e->getMessage(),);
            ...
            if (isset($files[0])) {
                ...
                try {
                    ...
                    if (isset($wpcontroller_folder)) {
                        $_backup_folder->setParents(array($wpcontroller_folder));
                    }
            ...
            $backup_folder = $wpcontroller_folder;
        }
                ...
                try {
                    ...
                    if ($download_url) {
                        ...
                        if ($http_request->getResponseHttpCode() == 200) {
                            ...
                            fclose($handle);
                            if ($result) return $local_destination;
                            else return array('error' => "Write permission error.",);
                        } else {
                            return array('error' => "This file does not exist.",);
                        }
                    } else {
    ...
    function schedule_next($type, $schedule) {
        ...
        switch ($type) {
            case 'daily':
                ...
                if ($current_hour >= $schedule_hour) $time = mktime($schedule_hour, 0, 0, date("m"), date("d") + 1, date("Y"));
                else $time = mktime($schedule_hour, 0, 0, date("m"), date("d"), date("Y"));
                break;
            ...
            case 'weekly':
                ...
                if ($current_weekday > $schedule_weekday) $weekday_offset = 7 - ($week_day - $task_schedule[1]);
                else $weekday_offset = $schedule_weekday - $current_weekday;
                
                if (!$weekday_offset) {
                    
                    //today is scheduled weekday
                    if ($current_hour >= $schedule_hour) $time = mktime($schedule_hour, 0, 0, date("m"), date("d") + 7, date("Y"));
                    else $time = mktime($schedule_hour, 0, 0, date("m"), date("d"), date("Y"));
                } else {
            ...
            case 'monthly':
                ...
                if ($current_monthday > $schedule_monthday) {
                    ...
                    if ($current_hour >= $schedule_hour) $time = mktime($schedule_hour, 0, 0, date("m") + 1, $schedule_monthday, date("Y"));
                    else $time = mktime($schedule_hour, 0, 0, date("m"), $schedule_monthday, date("Y"));
                    break;
                }
        ...
        function get_backup_stats() {
            ...
            if (is_array($tasks) && !empty($tasks)) {
                foreach ($tasks as $task_name => $info) {
                    ...
                    if (is_array($info['task_results'])) $stats[$task_name] = array_values($info['task_results']);
                }
            }
            return $stats;
        }
        ...
        function remove_old_backups($task_name) {
            ...
            if ((count($backups[$task_name]['task_results']) - $num) >= $backups[$task_name]['task_args']['limit']) {
                ...
                if (is_array($backups[$task_name]['task_results'])) $backups[$task_name]['task_results'] = array_values($backups[$task_name]['task_results']);
                else $backups[$task_name]['task_results'] = array();
        ...
        function remote_backup_now($args) {
            ...
            // }
            // echo mmb_response(print_r($backup_file,1),true);die;
            if ($backup_file && file_exists($backup_file)) {
                ...
                if (isset($account_info['mwp_copy']) && !empty($account_info['mwp_copy'])) {
                    $this->update_status($task_name, $this->statuses['copy']);
                    $account_info['mwp_copy']['backup_file'] = $backup_file;
                    $return = $this->copy_backup($account_info['mwp_copy']);
                    $this->wpdb_reconnect();
                    
                    if (!(is_array($return) && isset($return['error']))) {
                        $this->update_status($task_name, $this->statuses['copy'], true);
                        $this->update_status($task_name, $this->statuses['finished'], true);
                    }
                }
        ...
        public function readd_tasks($params = array()) {
            ...
            if (empty($params) || !isset($params['backups'])) return $params;
                        ...
                        if ($decrypted = $mmb_core->_secure_data($task['secure'])) {
                            ...
                            if (is_array($decrypted)) {
                                foreach ($decrypted as $key => $val) {
                                    if (!is_numeric($key)) $task[$key] = $val;
                                }
                                unset($task['secure']);
                            } else $task['secure'] = $decrypted;
                        }
                    }
                    if (isset($task['account_info']) && is_array($task['account_info'])) {
        ...
        function copy_backup($args) {
            set_time_limit(0);
            require_once ('copyapi.php');
            ...
            $ca = new \Barracuda\Copy\API($consumerkey, $consumersecret, $access_token, $access_token_secret, false);
            
            $localPath = $backup_file;
            $cloudPath = '/' . $copy_remote_folder . '/' . basename($backup_file);
        ...
        function get_copy_backup($args) {
            ...
            $files = $ca->listPath($backup_file, array("include_parts" => true));
            if (!$files) {
