...
class MMB_Installer extends MMB_Core
{
    ...
    function upgrade_wp($params){
        ...
        if($result){
            return mmb_response('wordpress upgraded successfully',true);
        }else{
            return mmb_response("sorry could not upgrade wordpress",false);
        }
    }
    ...
    function install_remote_file($params)
    {
        ...
        if (!$this->is_server_writable()) {
            return array(
                'error' => 'Failed, please add FTP details</a>'
            );
        }
        ...
        foreach ($package as $package_url) {
            ...
            $files = glob($destination.'/'.$key.'/*'); // get all file names
    ...
    function do_upgrade($params = null)
    {
        ...
        if (!$this->is_server_writable()) {
            return array(
                'error' => 'Failed, please <a target="_blank" href="http://wpcontroller.co.uk/user-guide/faq/my-pluginsthemes-fail-to-update-or-i-receive-a-yellow-ftp-warning">add FTP details</a>'
            );
        }
...
//			}
       ...
       if($args['search']){
        ...
        if (is_array($all_plugins) && !empty($all_plugins)) {
            ...
            foreach ($all_plugins as $path => $plugin) {
                if ($plugin['Name'] != 'Wpcontroller worker') {
        ...
        $upgradableplugins['upgradable'] =  (array)$this->get_upgradable_plugins();
        $plugins = array_merge($plugins,$upgradableplugins);
    ...
    function get_themes($args)
    {
        ...
        $upgradablethemes['upgradable'] =  (array)$this->get_upgradable_themes();
        $themes = array_merge($themes,$upgradablethemes);
        return $themes;
    }
