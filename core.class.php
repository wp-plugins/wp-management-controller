...
class MMB_Core extends MMB_Helper {
    ...
    function __construct() {
        ...
        $this->mmb_init_actions = array(
            ...
            'scheduled_optimize' => 'mwp_scheduled_optimize_tables',
            'schedule_remote_backup' => 'mwp_schedule_remote_backup',
            ...
            'clean_optimize_schedule' => 'mmb_clean_optimize_schedule',
            ...
            'upgrade_wp' => 'mmb_upgrade_wp',
            'wordfence_scan' => 'mmb_wordfence_scan',
            'wordfence_load' => 'mmb_wordfence_load',
            'sucuri_scan' => 'mmb_sucuri_scan',
            'buddy_backup_now' => 'mmb_buddy_backup_now',
            ...
            'get_broken_links' => 'mmb_get_broken_links',
        );
    ...
    function network_admin_notice() {
        echo '<div class="error" style="text-align: center;"><p style="color: red; font-size: 14px; font-weight: bold;">Attention !</p><p>
	  	Please add this site and your network blogs, with your network adminstrator username, to your <a target="_blank" href="http://wpcontroller.co.uk/wp-admin">WpController.co.uk</a> account now to remove this notice or "Network Deactivate" the Worker plugin to avoid <a target="_blank" href="http://wpcontroller.co.uk/user-guide/security">security issues</a>.	  	
	  	</p></div>';
    }
    ...
    function admin_notice() {
        echo '<div class="error" style="text-align: center;"><p style="color: red; font-size: 14px; font-weight: bold;">Attention !</p><p>
	  	Please add this site now to your <a target="_blank" href="http://wpcontroller.co.uk/wp-admin">WpController.co.uk</a> account.  Or deactivate the Worker plugin to avoid <a target="_blank" href="http://wpcontroller.co.uk/user-guide/security">security issues</a>.	  	
	  	</p></div>';
    }
    ...
    function support_page_dialog() {
        ...
        function get_wordfence_instance() {
            if (!isset($this->wordfence_instance)) {
                $this->wordfence_instance = new MMB_WORDFENCE();
            }
        ...
        function get_sucuri_instance() {
            if (!isset($this->sucuri_instance)) {
                $this->sucuri_instance = new MMB_SUCURI();
            }
            return $this->sucuri_instance;
        }

        /*
         * Get an instance of Piwik 
         */

        function get_piwik_instance() {
            if (!isset($this->piwik_instance)) {
                $this->piwik_instance = new MMB_PIWIK();
            }
            return $this->piwik_instance;
        }

        /*
         * Get an instance of Backupbuddy 
         */

        function get_backupbuddy_instance() {
            if (!isset($this->backupbuddy_instance)) {
                $this->backupbuddy_instance = new MMB_BuddyBackup();
            }
            return $this->backupbuddy_instance;
        }
        ...
        function uninstall($deactivate = false) {
            ...
            delete_option('mwp_schedule_remote_backup');
        ...
        function update_worker_plugin($params) {
            ...
            if ($download_url) {
                ...
                if (!$this->is_server_writable()) {
                    return array(
                        'error' => 'Failed, please <a target="_blank" href="http://wpcontroller.co.uk/user-guide/faq/my-pluginsthemes-fail-to-update-or-i-receive-a-yellow-ftp-warning">add FTP details for automatic upgrades.</a>'
                    );
                }
                ...
                if (is_wp_error($result) || !$result) {
                    return array(
                        'error' => 'WpController Worker plugin could not be updated.'
                    );
                } else {
                    return array(
                        'success' => 'WpController Worker plugin successfully updated.'
                    );
                }
            }
            return array(
        ...
        function automatic_login() {
            ...
            $itheme_section = isset($_GET['itheme_section']) ? $_GET['itheme_section'] : 0;
            ...
            if (isset($itheme_section) && $itheme_section != '0') {
                $this->itheme_redirect($itheme_section);
                exit;
            }
            
            // Getting redirect
            $redir = (string) isset($_GET['redir']) ? base64_decode($_GET['redir']) : '';

            if($redir != ''){
                wp_safe_redirect(admin_url($redir));
                die;
            }
        }
