...
class MMB_Helper {
    ...
    function mmb_set_sitemeta_transient($option_name, $option_value) {
        ...
        if ($this->mmb_get_sitemeta_transient($option_name)) {
            $result = $wpdb->update($wpdb->sitemeta, array(
    ...
    function authenticate_message($data = false, $signature = false, $message_id = false) {               
        ...
        if ((int) $current_message >= (int) $message_id)
            return array(
                'error' => $current_message . $message_id . 'Invalid message received. Deactivate and activate the WpController Worker plugin on this site, then re-add it to your WpController account.'
            );

        $pl_key = $this->get_master_public_key();
        if (!$pl_key) {
            return array(
                'error' => 'Authentication failed. Deactivate and activate the WpController Worker plugin on this site, then re-add it to your WpController account.'
            );
        }
        ...
        //     } else if ($verify == 0) {
        //         return array(
        //             'error' => 'Invalid message signature. Deactivate and activate the WpController Worker plugin on this site, then re-add it to your WpController account.'
        //         );
        //     } else {
        //         return array(
    ...
    function itheme_redirect($param) {
        //echo $param; exit;
        //High priority
        if ($param == 'change_admin_user')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_advanced'));
        else if ($param == 'brute_force_attack')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_brute_force_settings'));
        else if ($param == 'scheduled_db_backup')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_backup_enabled'));
        else if ($param == 'malware_scanning')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_malware_enabled'));
        else if ($param == 'strong_passwords')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_strong_passwords_enabled'));
        else if ($param == 'nickname')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_force_unique_nicename'));
        else if ($param == 'twofactor')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_pro#itsec_two_factor_enabled')); //Medium priority starts
        else if ($param == 'vulnerabilities')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_four_oh_four_enabled'));
        else if ($param == 'id_one')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_advanced#itsec_authentication_admin_user_userid'));
        else if ($param == 'available_24_7')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_away_mode_enabled'));
        else if ($param == 'change_detections')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_file_change_enabled'));
        else if ($param == 'default_address')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_hide_backend_enabled'));
        else if ($param == 'protecting_files')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_protect_files'));
        else if ($param == 'not_blocking_suspicious')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_suspicious_query_strings'));
        else if ($param == 'generator_tag')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_generator_tag'));
        else if ($param == 'user_agent')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_comment_spam'));
        else if ($param == 'attack_your_site')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_disable_xmlrpc'));
        else if ($param == 'execute_php')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_uploads_php'));
            ...
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_ssl_login'));
        else if ($param == 'not_disabled_directory')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_directory_browsing'));
        else if ($param == 'not_blocking_http')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_request_methods'));
        else if ($param == 'not_blocking_non_english')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_non_english_characters'));
        else if ($param == 'long_urls')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_long_url_strings'));
        else if ($param == 'config_writeable')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_write_permissions'));
        else if ($param == 'windows_live_writer')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_wlwmanifest_header'));
        else if ($param == 'really_simple_discovery')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_edituri_header'));
        else if ($param == 'plugin_themes')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_random_version'));
        else if ($param == 'edit_plugin_themes')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_file_editor'));
        else if ($param == 'unnecessary_information')
            wp_safe_redirect(admin_url('admin.php?page=toplevel_page_itsec_settings#itsec_tweaks_server_login_errors'));
